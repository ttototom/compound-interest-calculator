...
const CompoundInterestCalculator = () => {
  ...
  const resultRef = useRef(null); // 결과 영역을 참조하기 위한 ref 추가

  const formatNumber = (num) => num.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  const parseNumber = (str) => parseFloat(str.replace(/,/g, "")) || 0;

  const handleCalculate = () => {
    let prevBalance = parseNumber(initialInvestment);
  ...
  return (
    <div className="container">
      ...
      <div className="input-group">
        ...
        <input type="text" value={initialInvestment} onChange={(e) => setInitialInvestment(formatNumber(parseNumber(e.target.value)))} />
      </div>
      <div className="input-group">
        <label>Daily Return Rate (%)</label>
        <input type="text" value={dailyReturnRate} onChange={(e) => setDailyReturnRate(e.target.value)} />
      </div>
      <div className="input-group">
      ...
      <div className="input-group">
        ...
        <input type="text" value={withdrawalRate} onChange={(e) => setWithdrawalRate(e.target.value)} />
      </div>
      <div className="input-group">
